 ...
 void zeroPattern()
{
  for (int i=0; i<nbits; i++){
    digitalWrite(i+pinshift,LOW);
  }
}

int getPORTC(){
  char PIN[] = {A0,A1,A2,A3,A4,A5};
  int sum = 0;
  for (int i=0; i<7;i++){
 ...
 void loop() {
   if (Serial.available() > 0) {
     ...
     switch (inByte) {
       ...
       case 1 :
          if (waitForSerial(timeOut_)) {
            ...
            if (!blanking_){
              if (currentPattern_){
                setPattern((unsigned long) currentPattern_); 
              } else {
                zeroPattern();
              }
            //Serial.write( byte(1));
         ...
         if (patternLength_ > 0) {
           ...
           for (byte i = 0; i < repeatPattern_ && (Serial.available() == 0); i++) {
             for (int j = 0; j < patternLength_ && (Serial.available() == 0); j++) {
               ...
               setPattern((unsigned long) triggerPattern_[j]);
               Serial.println(triggerPattern_[j]);
    ...
    if (triggerMode_) {
      boolean tmp = getPORTD() & inPinBit_;
      if (tmp != triggerState_) {
        if (blankOnHigh_ && tmp ) {
          ...
          if (triggerNr_ >=0) {
            ...
            setPattern((unsigned long) triggerPattern_[sequenceNr_]);
            sequenceNr_++;
...
byte getPORTB(){
  int sum = 0;
  for (int i=0; i<nbits; i++){
    if (digitalRead(i+pinshift) == HIGH){
      sum += pow(2,i);
    }
  }
  return (byte) sum;
